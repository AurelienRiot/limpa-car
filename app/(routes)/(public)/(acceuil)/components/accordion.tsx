...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-left py-4 font-medium transition-all hover:underline [&[data-state=open]>svg.plus]:hidden [&[data-state=closed]>svg.minus]:hidden [&[data-state=closed]]:bg-primary [&[data-state=closed]]:text-primary-foreground duration-200 [&[data-state=open]]:bg-white",
        className
      )}
      {...props}
    >
